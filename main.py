...
LIVE_ACCESS_TOKEN = "eyJ0eXAiOiJKV1QiLCJrZXlfaWQiOiJza192MS4wIiwiYWxnIjoiSFMyNTYifQ.eyJzdWIiOiI1NUJBOVgiLCJqdGkiOiI2OWYyZDI3Y2M0Zjc5ZDQxNzc1ZGNmMGQiLCJpc011bHRpQ2xpZW50IjpmYWxzZSwiaXNQbHVzUGxhbiI6ZmFsc2UsImlhdCI6MTc3NzUyMTI3NiwiaXNzIjoidWRhcGktZ2F0ZXdheS1zZXJ2aWNlIiwiZXhwIjoxNzc3NTg2NDAwfQ.IbxJDQfOAOg4H9hPs7tystGog-Q5UM9LaWnOp26Z3Yc"  # <<-- Add your LIVE Upstox API token here (do not commit to Git)
...
def get_option_instruments():
    ...
    try:
        ...
        spot = get_spot_price()
        if spot is not None:
            filtered = [c.get('instrument_key') for c in contracts_cache if c.get('instrument_key') and abs(c.get('strike_price', 0) - spot) <= 1500]
            logger.info(f"Filtered to {len(filtered)} contracts near spot price {spot}")
            return filtered if filtered else [c.get('instrument_key') for c in contracts_cache[:150] if c.get('instrument_key')]
        else:
            logger.warning("Could not get spot price, returning first 150 contracts")
            return [c.get('instrument_key') for c in contracts_cache[:150] if c.get('instrument_key')]
    except Exception as e:
...
def get_live_oi_from_quotes(instrument_keys):
    ...
    """
    global last_oi_snapshot, last_oi_delta
    ...
    raw_delta_ce = ce_oi_total - last_oi_snapshot.get('ce', 0)
    raw_delta_pe = pe_oi_total - last_oi_snapshot.get('pe', 0)

    if abs(raw_delta_ce) > 0 or abs(raw_delta_pe) > 0:
        if last_oi_snapshot.get('ce', 0) != 0:
            last_oi_delta = {'ce': raw_delta_ce, 'pe': raw_delta_pe}
...
def calculate_vwap_rsi(df):
    ...
    # Standard Wilder's RSI Logic (RMA)
...
def check_signal_conditions(spot, day_open, vwap, rsi, oi_trend, oi_confirmation):
    ...
    if all(conditions['CE'].values()):
        signal = 'BUY CE'
    elif all(conditions['PE'].values()):
        signal = 'BUY PE'

    if signal and TRIGGER_OPPOSITE_SIGNAL:
        original_signal = signal
        # Swap Bullish signal (CE match) with Bearish trade (PE) and vice-versa
        signal = 'BUY PE' if original_signal == 'BUY CE' else 'BUY CE'
        logger.info(f"  🔄 OPPOSITE SIGNAL MODE: Swapping {original_signal} to {signal}")
...
{'-' * 85}
  CALL: {'✅' if ce['price_above_open'] else '❌'} Open  {'✅' if ce['price_above_vwap'] else '❌'} VWAP  {'✅' if ce['rsi_bullish'] else '❌'} RSI>60  {'✅' if ce['oi_bullish'] else '❌'} OI-Bull  →  {ce_result} {'(Trades PE via Opposite Mode)' if all(ce.values()) and TRIGGER_OPPOSITE_SIGNAL else ''}
  PUT:  {'✅' if pe['price_below_open'] else '❌'} Open  {'✅' if pe['price_below_vwap'] else '❌'} VWAP  {'✅' if pe['rsi_bearish'] else '❌'} RSI<40  {'✅' if pe['oi_bearish'] else '❌'} OI-Bear  →  {pe_result} {'(Trades CE via Opposite Mode)' if all(pe.values()) and TRIGGER_OPPOSITE_SIGNAL else ''}
"""
